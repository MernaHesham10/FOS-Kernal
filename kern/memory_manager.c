...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* PointerToPageTable = kmalloc(PAGE_SIZE), * PointerToPageTableTemp = PointerToPageTable;
	uint32 ReturenedPhysicalAddress = kheap_physical_address((unsigned int) PointerToPageTable);

	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(ReturenedPhysicalAddress, PERM_WRITEABLE | PERM_USER | PERM_PRESENT);

	unsigned int IndexInPageTable = 0;
	while(IndexInPageTable < PAGE_SIZE / 4) {
		*PointerToPageTable = 0;
		PointerToPageTable = PointerToPageTable + 1;
		IndexInPageTable = IndexInPageTable + 1;
	}
	tlbflush();
	return PointerToPageTableTemp;
}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 IndexInVirtualAddress = 0;
	virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE);
	size = ROUNDUP(size, PAGE_SIZE);

	while(IndexInVirtualAddress < size)	{
		pf_add_empty_env_page(e, (virtual_address + IndexInVirtualAddress), 1);
		IndexInVirtualAddress = IndexInVirtualAddress + PAGE_SIZE;
	}
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 IndexInVirtualAddress = 0, IndexInPageWS = 0;
	int checkPageTableEmptyOrNot = 0;
	uint32* PointerToDirectoryTable = NULL;

	while(IndexInVirtualAddress < size) {
		pf_remove_env_page(e, (virtual_address + IndexInVirtualAddress));

		while(IndexInPageWS < e -> page_WS_max_size) {
			if(env_page_ws_get_virtual_address(e, IndexInPageWS) == (virtual_address + IndexInVirtualAddress)) {
				env_page_ws_clear_entry(e, IndexInPageWS);
				break;
			}
			IndexInPageWS = IndexInPageWS + 1;
		}

		unmap_frame(e -> env_page_directory, (void*)(virtual_address + IndexInVirtualAddress));
		get_page_table(e -> env_page_directory, (void*)(virtual_address + IndexInVirtualAddress), &PointerToDirectoryTable);

		if(PointerToDirectoryTable != NULL) {
			for(int IndexInDirectoryTable = 0; IndexInDirectoryTable < 1024; IndexInDirectoryTable++) {
				if((uint32)(PointerToDirectoryTable[IndexInDirectoryTable] & PERM_PRESENT) == 1) {
					checkPageTableEmptyOrNot = 1;
					break;
				}
			}
			if(checkPageTableEmptyOrNot == 0) {
				uint32 getPhysicalAddress = kheap_physical_address((uint32)PointerToDirectoryTable);
				struct Frame_Info* PointerToFrameInfo = to_frame_info(getPhysicalAddress);
				PointerToFrameInfo -> references = 0;
				free_frame(PointerToFrameInfo);
				e -> env_page_directory[PDX(virtual_address + IndexInVirtualAddress)] = 0;
			}
		}
		IndexInVirtualAddress = IndexInVirtualAddress + PAGE_SIZE;
	}
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	while(src_virtual_address < (src_virtual_address+size)) {
			pf_add_env_page(e, dst_virtual_address,(void*)src_virtual_address);
			src_virtual_address = src_virtual_address + PAGE_SIZE;
