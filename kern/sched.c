...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	int IndexOfLevelsNumber , IndexOfReadyQueuesNumber;
	...
	for(IndexOfReadyQueuesNumber = 0; IndexOfReadyQueuesNumber < num_of_ready_queues; IndexOfReadyQueuesNumber++) {
		quantums[IndexOfReadyQueuesNumber] = quantumOfEachLevel[IndexOfReadyQueuesNumber];
	}

	IndexOfLevelsNumber = 0;
	while(IndexOfLevelsNumber < numOfLevels) {
		init_queue(&env_ready_queues[IndexOfLevelsNumber]);
		IndexOfLevelsNumber = IndexOfLevelsNumber + 1;
	}
...
struct Env* fos_scheduler_MLFQ()
{
	...
	int IndexOfReadyQueuesNumber;
	struct Env* environmentStrObject = NULL;

	//[1] If the current environment (curenv) exists, place it in the suitable queue
	if(curenv != NULL) {

		if(currentEnvironmentVar == num_of_ready_queues - 1) {
			enqueue(&env_ready_queues[currentEnvironmentVar], curenv);
		}
		else {
			enqueue(&env_ready_queues[currentEnvironmentVar + 1], curenv);
		}
	}
	...
	//	  Else, return NULL
	IndexOfReadyQueuesNumber = 0;
	for(; IndexOfReadyQueuesNumber < num_of_ready_queues; ) {

		environmentStrObject = dequeue(&env_ready_queues[IndexOfReadyQueuesNumber]);

		if(environmentStrObject != NULL) {
			currentEnvironmentVar = IndexOfReadyQueuesNumber;
			kclock_set_quantum (quantums[IndexOfReadyQueuesNumber]);
			return environmentStrObject;
		}
		IndexOfReadyQueuesNumber = IndexOfReadyQueuesNumber + 1;
	}
		 return NULL;
}
